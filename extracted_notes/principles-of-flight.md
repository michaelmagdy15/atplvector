...
- This principle, applied to aerodynamics, involves that the mass flow of air per unit time must remain constant at all times.
  ...
  - Therefore: A x V x ρ = Constant throughout a tube.Note: Density is considered constant and can be disregarded at low speeds (below Mach 0.4)
...
| APPLICATION OF THE BERNOULLI'S THEOREM IN AVIATION· An aerofoil is a specifically shaped object intended to interact with airflow in a certain way.Wings are aerofoils designed to accelerate airflow above it.Because airflow's speed is increased, its static pressure must be decreased.As a consequence, a resulting force is created. This force is known as lift. |  |
| --- | --- |
...
| · Laminar (streamline) airflowStreamline flow exists when succeeding molecules follow a steady path.At any given point, the molecules will experience the same velocities and pressure as the preceding molecules.Streamlines separate to indicate lower velocity, and narrow to indicate an increase in velocity. |  |
| --- | --- |

| · Turbulent airflowHappens when the airflow doesn't follow a steady path.After turbulent airflow, streamlines are broken.At a given point, molecules will experience different velocities and pressures compared to the preceding molecules.Flow is unsteadyThis results in wasted energy and an increase of drag |  |
| --- | --- |
...
| · Lift formulaLift = (½ ρ v²) s CL = Q S CLρ = Density. Affected by variations in altitude.V = Velocity (TAS).S = Wing surface.CL = Coefficient of Lift.Q = Dynamic pressure. |  |
| --- | --- |
...
- Two main sections
  - Positive Pressure Gradient (first section).Air flows from a high static pressure area to a low static pressure area.Following the airflow, velocity increases and static pressure decreases.This is the natural flow of air, thus, air follows this gradient easily.
  ...
  - Adverse Pressure Gradient (second section).This gradient goes against the natural flow of air.Along the chord, air velocity reduces and static pressure increases.Air must have high energy in order to continue flowing against and adverse gradient.Once energy falls below a given threshold, airflow separates from the aerofoil.
...
| WINGTIP VORTICES· What are wingitp vortices?Wingtip vortices are circular patterns of rotating air left behind a wing as it generates lift.· What causes wingtip vortices?Wingtip vortices are caused by pressure differential at the lower and upper surfaces of the wing.This pressure differential creates a spanwise flow of air that tends to go around the wingtip towards the upper section of the wing.As a consequence, air flows from the underside of the wing, around the wingtip, towards the upper side. |  |
| --- | --- |
...
| · Spanwise lift distributionLift is less towards tips and greater at wing root.This is because at the wing root, the effect of vortices are almost negligibleHowever, at the wingtips the downwash greatly reduces effective angle of attack and mainly generates dragThe effect of vortices increases with low aspect ratio airplanesHigh aspect ratio have a very short chord line, not giving the vortices enough space to grow |  |
| --- | --- |
...
| · Wing shape, effect of vortices and spanwise liftRectangular wing produces much larger vortex than a tapered section. This is because larger tip chord allows more air to flow to the upper surface.Elliptical wings have a constant effective Angle of Attack. Virtually, it has no tip chord vortices, but they are too expensive and difficult to be manufactured.Tapered wing is a good compromise between both. |  |
| --- | --- |
...
| PRINCIPLE· Ground effectGround surface restricts the downwash created by wing vorticesTherefore, induced Angle of Attack decreases, increasing Effective Angle of AttackStall Angle of Attack is reduced because now the Effective AoA is bigger.Takes effect within a wingspans lengthBecomes noticeable within 1/2 wingspan80% of induced drag is lost during ground effect |  |
| --- | --- |
...
| · Wing loadingLF = Lift / WeightDuring turns and certain maneuvers, load factor is more than 1 since wings have to create more lift than the aircraft weights.Whenever load factor increases, stall speed increasesNew Vs = Vs1g x √ (LoadFactor) |  |
| --- | --- |

| · TurningDuring turns, lift is tilted to one side, reducing the vertical component of lift.To maintain straight and level, the AoA has to be increased to generate more liftDuring turns, load factor = (1 / cos ϕ)Stall speed during a turn = Stall speed normal x √ (1 / cos ϕ) |  |
| --- | --- |

| · Aircraft weightAs weight increases, stall speed increases. |  |
| --- | --- |
...
| · Leading edge slatsThey create a gap (slot) that re-energizes the Boundary Layer, delaying airflow separationLE slats increase stall AoAUsually located between engine and wing tipSlats do not change camber, nor wing surfaceNote that the usage of slats doesn't affect generated lift up to the critical angle of attack, but beyond it. |  |
| --- | --- |
...
| · Climb angleThe climb angle depends on excess thrust and weightMaximum angle of climb happens when an aircraft is flown at a speed of maximum excess thrust.The speed that provides a maximum climb angle is known as "Vx".This speed is used to sort close-in obstacles.For piston engines, this is approximately Vmp (maximum power). Note that power =/= dragFor jet engines, this is Vmd· The climb angle has some relationship with climb gradientClimb gradient = (Height Gain / Horizontal distance) x 100 (%)Climb gradient = Climb angle x 100 (%) |  |
| --- | --- |
...
| · Forces involved in a turnFor a turn to occur, a centripetal force, perpendicular to the aircraft's flight path, must exist.This centripetal force, which points towards the centre of the turn, is created by rolling the aircraft into the turn.When this happen, the lift force is divided into two componentsVertical component of lift must balance the weight in order to maintain level flight.Therefore, L cos φ = WHorizontal component of lift causes the aircraft trajectory to turn.· Load FactorDuring a turn, lift is tilted, thus reducing the vertical component of liftAn increase in total lift therefore is required in order to maintain altitudeBecause lift is greater than weight, Load Factor is greater than 1.Load Factor = 1 / cos Φ |  |
| --- | --- |
...
| · LiftUp to point B, this is the subsonic range. Aerofoil generates more lift with speed.At point C, a shockwave forms over the top surface. Drag starts to kick in.At point D, the airplane is at the transonic range. The shock wave promotes boundary layer separation, reducing lift generating capabilities of the aerofoil. This is known as shock stall.At poing E, the shockwave has moved to the trailing edge and separation no longer occurs over the wing. This is the point of maximum drag.At point F, the aeroplane is pretty much supersonic. A bow wave has appeared, and drag is also greatly reduced. |  |
| --- | --- |

| · DragAt Mcrit, as the shock wave is formed, drag starts increasing.At the drag-divergence Mach number, the drag starts to increase rapidly as the Mach number continues to increase. |  |
| --- | --- |
