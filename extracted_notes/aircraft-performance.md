...
| --- | --- |
| Gross performance | Average performance that a fleet flown by pilots should achieved if maintained and operated according to the airplane manualsGross performance is less than measured performance |
...
| · ThrustJet enginesThrust decreases slightly with speed, and starts to recover after Ram Recovery speed.However, it may be assumed to be constant to simplify explanations.Fixed-pitch propellersThrust decreases remarkably with increasing airspeedVariable-pitch propellersThrust remains more or less constant regardless of airspeed |  |
| --- | --- |
...
| FACTORS AFFECTING THRUST· AltitudeAs altitude increases, air density decreases, and so does available thrust. |  |
| --- | --- |

| FACTORS AFFECTING DRAG· WeightAs weight increases, drag also increases for a given airspeedAs weight increases, the speed of minimum drag increasesNew Vmd = Old Vmd x √(New weight / Old weight)· SpeedFor a given weight at a given level, drag varies with speed.The speed at which minimum drag occurs is known as minimum drag speed - Vmd |  |
| --- | --- |

| ACHIEVABLE SPEED· Achievable cruise speed rangeAt the minimum speed that can be maintained in flight, thrust available equals dragAt the maximum speed that can be maintained in flight, thrust available also equals dragBecause, with altitude, thrust available changes, the cruise speed range is reduced. |  |
| --- | --- |
...
| · The four take-off segmentsSegment 1From screen height to gear retractionAircraft keeps climbing at V2Segment 2From gear retraction until airplane levels off to accelerateAirlines specify a minimum altitude for level acceleration, which shall never be less than 400 ftSegment 3From level off until airplane is clean and able to resume climbThe main objective is to clean the aircraftSegment 4Thrust set to maximum continuous thrustAirplane continues to climb to 1500 ft AAL or MSA |  |
| --- | --- |
