...
// Validate all required env vars are present at startup (log warnings only)
for (const key of requiredEnvVars) {
  if (!import.meta.env[key]) {
    console.warn(`Environment variable ${key} is not set. Using built-in fallback configuration.`);
  }
}
